...
export default function NavBarMobile() {
  ...
  return (
    <div className="w-full text-gray-700 bg-cream md:hidden relative">
      {/* Overlay */}
      {open && (
        <div
          className="fixed top-25 inset-0 bg-black/40 bg-opacity-40 z-30 transition-opacity duration-300 md:hidden"
          onClick={() => setOpen(false)}
          aria-hidden="true"
        />
      )}
      <div className="flex flex-col max-w-7xl px-8 mx-auto md:items-center md:justify-between md:flex-row relative z-50">
        <div className="flex flex-row items-center justify-between py-6 relative z-50">
          ...
          <button
            className="rounded-lg md:hidden focus:outline-none focus:shadow-outline focus:ring-2 focus:ring-yellow focus:ring-offset-2"
        ...
        <nav
          id="mobile-menu"
          className={`absolute left-0 top-full w-full bg-cream flex flex-col grow md:items-center pb-4 md:pb-0 md:flex md:justify-end md:flex-row origin-top transition-all duration-300 overflow-hidden z-40
            ${
              open
                ? "h-[350px] opacity-100"
                : "h-0 opacity-0 pointer-events-none"
            }
            md:static md:h-auto md:opacity-100 md:pointer-events-auto md:bg-transparent`}
          role="navigation"
